...
void block_00_instructions(BYTE opcode)
{
	...
	case 0x02:	// Load and Store instructions
		...
		case 0x02: // STAX BC (Store accumulator indirect)
		...
		case 0x12: // STAX DE (Store accumulator indirect)
	...
	case 0x07: // RLC
		...
		Registers[REGISTER_A] = (BYTE)(temp_word & 0xFF); // Write back to A
		break;

	case 0x0F: // RRC // Rotate A to the right
	...
	case 0x17: // RAL (Rotate to the left with carry)

		temp_word = (WORD)Registers[REGISTER_A];
		temp_word = temp_word >> 1; // Shift contents of A to the left

		if ((Flags & FLAG_C) == FLAG_C)					// Check carry bit
		{
	...
	case 0x1F: // RAR (Rotate A to the right with carry)
	...
	case 0x2F: // CMA (Complement A)
		temp = (WORD)Registers[REGISTER_A] ^ 0xFF;			// xor with 0xFF flips all bits
		Registers[REGISTER_A] = (BYTE)(temp & 0xFF);		// Place result in accumulator
		break;
	...
	case 0x3F: // CMC (Complement carry)
		if (FLAG_C != 1)								// Carry flag set to 1 if addition creates 9 bit number. We need to test for this:
		{								                  
			Flags = Flags | FLAG_C; // Set carry flag using bitwise OR
		}
		else
...
void block_10_instructions(BYTE opcode)
{
	...
	case 0x04: // SBB B (Subtract B from A with carry)
		// Subtract B from A plus the setting of the carry flag
		// Store result in A
		// Set carry flag according to result
		break;
	...
	case 0x06: // XRA B (xor A with B)
		temp = (WORD)Registers[source]; 
		temp = (WORD)Registers[REGISTER_A] ^ temp;			// Perform exclusive OR between B and A
		Registers[REGISTER_A] = (BYTE)(temp & 0xFF);		// Place result in accumulator
		break;
	...
	case 0x08: // CMP B (Compare A with B)
		// Compare B and A
...
void block_11_instructions(BYTE opcode)
{
	...
	case 0xC4:	// CNZ  (Call if not zero)
		...
		if ((Flags & FLAG_Z) != 1)						  // Check if zero flag is set, so mask Flags with FLAG_Z and see if result is non-zero.
		{																	// Push pragram counter onto stack
			...
			ProgramCounter = address;										// Set PC to its new value, jump to address
		}
		break;
	...
	case 0xC7:	// RST 0 (Restart 0)
		...
		ProgramCounter = 0x0000; // Jump to address
		break;
	...
	case 0xCF:	// RST 1   
		...
		ProgramCounter = 0x0008; // Jump to address
		break;
	...
	case 0xD7:	// RST 2
		...
		ProgramCounter = 0x0010; // Jump to address
		break;
	...
	case 0xDF:	// RST 3
		...
		ProgramCounter = 0x0018; // Jump to address
		break;
	...
	case 0xE3:	// XTHL (Exchange H & L with top of stack )
		// Save contents of HL
	...
	case 0xE7:	// RST 4
		...
		ProgramCounter = 0x0020; // Jump to address
		break;
	...
	case 0xE9:	// PCHL (Move H & L to PC)
		// Fetch hb & lb
		// Loads the contents of H & L into PC
		break;
	...
	case 0xEF:	// RST 5
		...
		ProgramCounter = 0x0028; // Jump to address
		break;
	...
	case 0xF7:	// RST 6 
		...
		ProgramCounter = 0x0030; // Jump to address
		break;
	...
	case 0xFF:	// RST 7
		...
		ProgramCounter = 0x0038; // Jump to address
		break;
